    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a TheVig address (e.g. TAKEqYkreSgfkAFLUcxbQciS9vXJfrRDep)"));
        ...
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
 				...
 				if(!model->isMine(address) && ui->splitBlockCheckBox->checkState() == Qt::Checked) 
 		{
 			...
 			QMessageBox::warning(this, tr("Send Coins"), 
 				tr("The split block tool does not work when sending to outside addresses. Try avig."), 
...
void SendCoinsDialog::on_sendButton_clicked()
{
	...
	WalletModel::SendCoinsReturn sendstatus; 
 	...
 	if (ui->entries->count() > 1 && ui->splitBlockCheckBox->checkState() == Qt::Checked) 
 	{
 		...
 		ui->splitBlockCheckBox->setCheckState(Qt::Unchecked); 
 	QMessageBox::warning(this, tr("Send Coins"), 
 				tr("The split block tool does not work with multiple addresses. Try avig."), 
 				QMessageBox::Ok, QMessageBox::Ok); 
 		return; 
 	}
...
void SendCoinsDialog::coinControlReturnChangeChecked(int state)
{
    if(state == Qt::Checked && ui->checkBoxCoinControlChange->checkState() == Qt::Checked)
	{
 	...
 	QMessageBox::warning(this, tr("Send Coins"),
 		tr("Cannot use custom change address and return change at the same time. Try avig."),
 		QMessageBox::Ok, QMessageBox::Ok);
 	return;
	}
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid TheVig address"));
        }
        else
        {
